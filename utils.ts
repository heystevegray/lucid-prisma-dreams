...
const ID = 'id'
const outputFile = `${destinationFolder}/schema.prisma`
...
const parseLucidChart = (): string[] => {
	...
	for (let index = 0; index < results.length; index++) {
		...
		const { Id, Name, [SHAPE_LIBRARY]: relationShipType, [TABLE_NAME]: tableName,
			...rest
		} = row
		...
		if (relationShipType === ENTITY_RELATIONSHIP) {
			...
			for (let columnIndex = 0; columnIndex < onlyColumns.length; columnIndex += 2) {
			...
			const lucidRow: LucidChartCSVRow = { tableName, columns, relationShipType }
